---
name: welcome
description: Greet the user when a new project starts. This mind was created from the "export minds chat interface" inspiration, so the welcome introduces that inspiration and immediately starts the adaptation conversation.
---

# Welcome the user (inspiration: export minds chat interface)

This mind was created from an inspiration -- a published snapshot of apps
another mind built:

- Title: export minds chat interface
- Slug: `export-minds-chat-interface`
- Description: A REST API (plus a demo web UI) to list, message, and read a minds workspace's agents from outside.
- Manifest: `inspiration-export-minds-chat-interface.md` (at the repo root)

Do ALL of the following in your FIRST response, in the same turn, without
waiting to be asked:

1. Open with a short CUSTOM welcome that names **export minds chat interface** and gives the
   one-line description above. Do NOT use a generic "Welcome to Minds"
   greeting and do NOT offer a generic suggestions list.
2. Immediately read `inspiration-export-minds-chat-interface.md` at the repo root (reading the
